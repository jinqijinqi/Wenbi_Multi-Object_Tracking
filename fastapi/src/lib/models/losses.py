...
class CIoULoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.ciou_loss = giou_loss

    def forward(self, iou, mask, ind, target):
        pred_iou = _tranpose_and_gather_feat(iou, ind)
        mask_iou = mask.unsqueeze(2).expand_as(pred_iou).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
        gt_bbox_xys=target
        centers_x = (gt_bbox_xys[:, :, 0:1] + gt_bbox_xys[:, :, 2:3]) / 2.0
        centers_y = (gt_bbox_xys[:, :, 1:2] + gt_bbox_xys[:, :, 3:4]) / 2.0
        x1 = centers_x - pred_iou[:, :, 0:1]
        y1 = centers_y - pred_iou[:, :, 1:2]
        x2 = centers_x + pred_iou[:, :, 2:3]
        y2 = centers_y + pred_iou[:, :, 3:4]
        pred_boxes = torch.cat([x1, y1, x2, y2], dim=-1)
        # loss = self.ciou_loss(pred_boxes * mask_iou, target * mask_iou, reduction="sum")
        loss = self.ciou_loss(pred_boxes, gt_bbox_xys, mask_iou, avg_factor=mask_iou.sum() + 1e-4)
        return loss
