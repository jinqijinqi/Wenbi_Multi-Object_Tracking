...
from models.losses import FocalLoss, TripletLoss,IOUloss
...
class MotLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            reg_targets=
            loss_iou = (self.iou_loss(bbox_preds.view(-1, 4), reg_targets)).sum() / opt.num_stacks
            loss_obj = (self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)).sum() / opt.num_stacks
