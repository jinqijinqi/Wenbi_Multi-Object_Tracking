...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __init__(self, opt, root, paths, img_size=(1088, 608), augment=False, transforms=None):
        ...
        self.augment = augment
        self.transforms = transforms
        self.heatmap_radius=_CornerNetRadius()
    ...
    def __getitem__(self, files_index):
        ...
        for k in range(min(num_objs, self.max_objs)):
            ...
            if h > 0 and w > 0:
                ...
                radii = self.heatmap_radius(w, h)
                update_heatmap(hm[cls_id], ct_int, radii, num_classes)
                if self.opt.ltrb:
...
def update_heatmap(heatmap: torch.Tensor, centers: np.ndarray, radii, label):
        out_h, out_w = heatmap.shape
        (cx, cy), (rx, ry), label = centers, radii,label
        ...
        grid_y = torch.arange(-ry, ry + 1).view(-1, 1)
        grid_x = torch.arange(-rx, rx + 1).view(1, -1)
        ...
        masked_heatmap = heatmap[cy - t:cy + b, cx - l:cx + r]
